...
export class HydrogenNotebook {
  ...
  constructor(notebooks, editor) {
    ...
    if (editorPath && path.extname(editorPath) === '.ipynb') {
      ...
      // TODO TODO v2
      //  - TODO Add a corresponding .destroy() to set the buffer back to a normal file
      const ipynbPyFile = IpynbPyFile.installIntoTextBuffer(this.editor.buffer);
  ...
  destroy() {
    ...
    if (this.ipynbSync) this.ipynbSync.destroy();
  ...
  deleteMarkersByFinds(propertiess) {
    ...
    this.emitter.emit('did-update-outputs');
  }

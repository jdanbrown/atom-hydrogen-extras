  ...
  addCommands() {
    this.disposables.add(
      atom.commands.add('atom-workspace', {
        ...
        'hydrogen-extras:new-term-with-glances': ev => newTermWithCommand('glances -t1 -b -1 -4 --process-short-name'),
      }),
...
export class HydrogenNotebook {
  ...
  destroy() {
    if (this.kernel) this.shutdownKernel();
    this.disposables.dispose();
  }
  ...
  async runCode(options, range) {
    ...
    // Strip ipynb-metadata-bearing cell delims, e.g.
    //  - '##'
    //  - '## Foo'
    //  - '%%'
    //  - '%% {scrolling: false}'
    ...
    // Handle '%%' case where code is empty, which is difficult to handle in the regex
    if (code === '%%') code = '';

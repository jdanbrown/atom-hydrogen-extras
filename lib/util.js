...
export function parseCellMetadata(cellText) {
  ...
  //  - `%%`                  -> {metadata: {}, ...}
  //  - `%%timeit -n10`       -> {metadata: {}, ...}
  const {delim, delimText, body} = _parseCell(cellText);
  let metadata;
  if (delimText) {
...
export function _parseCell(cellText) {
...
export function cellHasMetadataLine(cellText) {
  const {delim} = _parseCell(cellText);
  return !!delim;
}

export function cellHasCellMagic(cellText) {
  return !!cellText.match(/^%%\S/);
}

export function cellHasCode(cellText) {

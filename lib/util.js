...
export function writableBuffer(onFinish) {
  ...
  return new Writable({
    ...
    console.debug('writableBuffer: Writable.finish');
...
// HACK Make a TextBuffer.loadSync that works like .load, where it accepts file|path instead of just path
export function textBufferLoadSync(source, params = {}) {
  // Copied with minor tweaks from https://github.com/atom/text-buffer/blob/master/src/text-buffer.coffee
  const buffer = new TextBuffer(params);
  if (typeof(source) === 'string') {
    console.warn('setPath'); // XXX
    buffer.setPath(source);
  } else {
    console.warn('setFile'); // XXX
...
// HACK for buildTextEditor, mimicking https://github.com/atom/atom/blob/a1b0340/src/text-editor.js#L187
export function getTextMateLanguageMode() {
  const buffer = new TextBuffer();
  atom.grammars.autoAssignLanguageMode(buffer);
  return buffer.languageMode.__proto__.constructor;
}

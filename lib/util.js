...
export const packageName = 'hydrogen-extras';

export function parseCellMetadata(cellText) {
  // Examples:
  //  - `%% {x: foo, y: bar}` -> {metadata: {x: 'foo', y: 'bar'}, ...}
  //  - `%%`                  -> {metadata: {}, ...}
  //  - `%%timeit -n10`       -> {metadata: {}, ...}
  const {delim, delimText, body} = _parseCellMetadata(cellText);
  let metadata;
  if (delimText) {
...
export function _parseCellMetadata(cellText) {
  const [, delim, delimText, body] = cellText.match(/^(%% *(?: +(.*))?)(?:\n((?:.|\n)*))?$/) || [
    null, null, null, cellText,
  ];
  return {delim, delimText, body};
}

export function dumpCellMetadataLine(metadata) {
...
export function cellHasMetadataLine(cellText) {
  return !!_parseCellMetadata(cellText).delim;
}

// Assumes magic is fine line in cellBody, i.e. cell metadata (`%% ...`) has already been parsed away
export function parseCellMagic(cellBody) {
  // Examples:
  //  - `%%time\nfoo`        -> {magic: 'time',        code: 'foo'}
  //  - `%%timeit -n10\nfoo` -> {magic: 'timeit -n10', code: 'foo'} [TODO Parse magicArgs separate from magic?]
  //  - `foo`                -> {magic: null,          code: 'foo'}
  let magic, code;
  [, magic, code] = cellBody.match(/^%%(\S+)(?:\n((?:.|\n)*))?$/) || [
    null, null, cellBody,
  ];
...
export function cellHasCellMagic(cellBody) {
  return !!parseCellMagic(cellBody).magic;
}

// A cell is empty if it has no magic or code, ignoring the cell metadata line
export function cellIsEmpty(cellText) {
  const {body} = parseCellMetadata(cellText);
  return !body.trim();
}

export function parsePyMetadata(pyMetadataText) {
  const {magic, code} = parseCellMagic(pyMetadataText);
  if (magic === 'metadata') {
    return dropExceptions(() => yaml.safeLoad(code)); // Map parse errors to undefined
  }
}
...
export function puts(x) {
  console.log('puts', x);
  return x;
}

// e.g. for stuffing true/false into a DOM Element.dataset
export function boolishString(x) {
  return x ? 'truthy' : '';
}
...
export function dropExceptions(f) {
  try {
    return f();
  } catch (e) {
    return undefined;
...
export function newTermWithCommand(command) {

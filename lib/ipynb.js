...
export class IpynbSync {
  ...
  async start() {
    this.disposables.add(
      ...
      // TODO Still needed? See comment at ensureGrammar
      this.editor.buffer.onDidChangePath(() => this.ensureGrammar()),
  ...
  //  - Workaround: this.ensureGrammar() at late points during file load (e.g. in reloadOutputsFromIpynb)
  ensureGrammar() {
    this.autoDetectGrammar();
    // console.debug('ensureGrammar:', {scopeName: this.editor.getGrammar().scopeName}); // XXX dev
  }

  autoDetectGrammar() {
    // Mimic the behavior of 'Grammar Selector: Show' -> 'Auto Detect'
    //  - https://github.com/atom/atom/blob/01c4017/packages/grammar-selector/lib/grammar-list-view.js#L50
    atom.grammars.autoAssignLanguageMode(this.editor.buffer);
  }

  clearGrammar() {
    // Clearing the grammar will result in 'Null Grammar'
    this.editor.setGrammar(null);
  }
    ...
    try {
      ...
      if (!ipynbSource) {
        ipynbSource = this.jsonStringifyLikeJupyter({
          cells: [],
          metadata: {
            kernelspec: this.defaultKernelspecForGrammar(),
          },
          ...this.nbformats,
        });
      }
      ...
      const kernelspec = ipynbMetadata.kernelspec || this.defaultKernelspecForGrammar();
      pyLines.push(
        '%%metadata',
  ...
  reloadOutputsFromIpynb() {
    ...
    try {
      ...
      // HACK Manually trigger vega listeners for responsive plot sizing
      //  - See comment in KernelResultsReceiver.resultAppendVega
      vegaUtil.manuallyTriggerWindowResize();

      // Ensure editor grammar is .py before allCodeChunks, which is sensitive to e.g. comment chars
      //  - Also see HACK comment at ensureGrammar
      this.clearGrammar(); // HACK Else syntax highlighting is absent for slow-to-load files, even though grammar is correct
      this.ensureGrammar();
      ...
      let {ipynbCells, kernelspec, oldIpynbHash, newIpynbHash, savedIpynbHash} = {};
      ...
      if (stashIpynbDataOnRead) {
        ({ipynbCells, kernelspec, oldIpynbHash, newIpynbHash} = stashIpynbDataOnRead);
      } else {
        ...
        if (ipynbSource) {
          // TODO De-dupe with getPyFromIpynbSource (logic at top)
          const ipynb = JSON.parse(ipynbSource);
          ipynbCells = ipynb.cells;
          kernelspec = (ipynb.metadata || {}).kernelspec || this.defaultKernelspecForGrammar();
          oldIpynbHash = this.getPyMetadataIpynbHash(); // TODO O(n) -> O(1) (see below)
    ...
    try {
      ...
      ipynb.metadata = {
        ...
        kernelspec: {
          ...
          name:         kernel.name || kernel.displayName || this.defaultKernelspecForGrammar().name,
          display_name: kernel.displayName                || this.defaultKernelspecForGrammar().display_name,
          language:     kernel.language                   || this.defaultKernelspecForGrammar().language,
        },
      };
  ...
  defaultKernelspecForGrammar(grammar = null) {
    grammar = grammar || this.editor.getGrammar();
    const language = (grammar
      ? _.last(grammar.scopeName.split('.')) // HACK How to robustly get some kind of language id from a grammar?
      : 'python'
    );
    return {

...
export class IpynbSync {
  ...
  async start() {
    this.disposables.add(
      ...
      this.editor.buffer.onDidChangePath(() => this.setGrammar()), // TODO Still needed? See comment at function def.
  ...
  reloadOutputsFromIpynb() {
    ...
    try {
      ...
      //  - Also see HACK comment at function def
      this.setGrammar();
      ...
      // Reuse intermediates from getPyFromIpynbSource, so we don't have to re-parse the json, which could be very slow
      //  - HACK Action at a distance! (<- getPyFromIpynbSource)
      let {ipynbCells, oldIpynbHash, newIpynbHash} = this.lastIpynbStash || {};
      delete this.lastIpynbStash; // Don't re-consume this on reload // TODO TODO Dubious...
      if (!ipynbCells) {
      ...
      // TODO TODO Need _old_ and new ipynb_hash, from getPyFromIpynbSource
      ...
      if (hasOutputs && oldIpynbHash === newIpynbHash) {
        console.debug(`newIpynbHash[skip]: hasOutputs[${hasOutputs}], ${(oldIpynbHash || '').slice(0, 7)} -> ${(newIpynbHash || '').slice(0, 7)}`); // XXX
        // Yay, no work to do!
        return;
      } else {
        console.debug(`newIpynbHash[RELOAD]: hasOutputs[${hasOutputs}], ${(oldIpynbHash || '').slice(0, 7)} -> ${(newIpynbHash || '').slice(0, 7)}`); // XXX
      }
  ...
  getIpynbFromPyEditor() {
    // console.debug('getIpynbFromPyEditor'); // XXX dev
    try {
      ...
      ipynb.metadata = {
        ...
        kernelspec: {
          ...
          name: kernel.name || kernel.displayName || 'python',
          display_name: kernel.displayName || 'python',
  ...
  getPyMetadataIpynbHash(pyCells = null) {
    pyCells = pyCells || allCodeChunks('cell', this.editor)[0]; // TODO O(n) -> O(1) by parsing for first cell only

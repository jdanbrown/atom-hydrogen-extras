...
const {
  ifPackageActive, getTextMateLanguageMode, joinIfArray, onAnyEvent, sha1hex, stripSuffix,
  textBufferLoadSync, withException,
} = require('./util');

export class IpynbSync {

  // TODO TODO
  static open(notebooks, uri, disposables) {
    console.debug('IpynbSync.open', uri);

    const TextMateLanguageMode = getTextMateLanguageMode(); // HACK (see util.js)

    window.notebooks = notebooks; // XXX
    window.uri = uri; // XXX
    window.TextBuffer = TextBuffer; // XXX
    window.IpynbSync = IpynbSync; // XXX
    window.IpynbPyFile = IpynbPyFile; // XXX
    const ipynbSync = new IpynbSync(notebooks);
    const ipynbPyFile = new IpynbPyFile(uri, ipynbSync);

    const buffer = textBufferLoadSync(ipynbPyFile); // TODO TODO This maybe also works?
    // const buffer = TextBuffer.loadSync(uri); // TODO TODO This works, safely
    // HACK Mimic https://github.com/atom/atom/blob/a1b0340/src/text-editor.js#L187
    // buffer.setLanguageMode(null); // XXX This causes weird errors
    buffer.setLanguageMode(new TextMateLanguageMode({buffer, config: atom.config}));
    buffer._ipynbPyFile = ipynbPyFile; // For HydrogenNotebook.toggleIpynbPyFile
    window.buffer = buffer; // XXX

    const editor = atom.workspace.buildTextEditor({buffer, autoHeight: false});
    window.editor = editor; // XXX
    ipynbSync.setEditor(editor);
    ipynbSync.start().then(x => disposables.add(x));
  ...
  // TODO TODO Fold back into constructor
  setEditor(editor) {
    this.editor = editor;
  }
  ...
  get notebook() {
    return this.notebooks.getNotebookForTextEditor(this.editor);
  }
  ...
  async start() {
    this.disposables.add(
      onAnyEvent(this.editor.buffer, eventName => console.info('this.editor.buffer', eventName)), // XXX
      // TODO TODO Re-enable
      ...
      // await this.ipynbPyFile.start(), // TODO TODO (then make not async)
    );

...
const {getCellRanges, getCellRange, getCellPoint} = require('./cells');

const packageName = 'hydrogen-extras'

window.hydrogen = hydrogen // XXX dev
window.getCellRanges = getCellRanges // XXX dev
window.getCellRange = getCellRange // XXX dev
window.getCellPoint = getCellPoint // XXX dev
...
const notebookHydrogen = {
  ...
  activate() {
    ...
    this.disposables.add(
      onPackageByName('vim-mode-plus', vmp => {
        this.vmp = vmp;
  ...
  addCommands() {
    this.disposables.add(
      atom.commands.add('atom-workspace', {
        ...
        'hydrogen-extras:toggle-pythonstartup': ev => this.getActiveNotebook().togglePYTHONSTARTUP(),
        ...
        'hydrogen-extras:run-all-cells': ev => this.getActiveNotebook().runAllCells(),
        'hydrogen-extras:run-all-cells-above': ev => this.getActiveNotebook().runAllCellsAbove(),
        'hydrogen-extras:run-all-cells-below': ev => this.getActiveNotebook().runAllCellsBelow(),
        'hydrogen-extras:run-all-cells-above-and-current': ev => this.getActiveNotebook().runAllCellsAboveAndCurrent(),
        'hydrogen-extras:run-all-cells-below-and-current': ev => this.getActiveNotebook().runAllCellsBelowAndCurrent(),
  ...
  getNotebookForTextEditor(editor) {
    return this.notebooks[editor.id];
  },

  async toggleKernelMonitor() {
    // Noop if hydrogen isn't active, since you'd get a blank editor window instead of the monitor
...
class NotebookHydrogen {
  ...
  interruptKernel() {
    ...
    if (!kernel) {
      ...
      atom.notifications.addInfo(`kernel[${kernel.kernelSpec.display_name}]: ^C`);
      kernel.interrupt();
    }
  }
  ...
  //  - Duplicates: hydrogen.main.handleKernelCommand
  shutdownKernelAndDeleteAllResults() {
    const {kernel} = hydrogen.store;
    if (!kernel) {
      atom.notifications.addWarning('No kernel running');
    } else {
      atom.notifications.addInfo(`kernel[${kernel.kernelSpec.display_name}]: Shutting down`);
  ...
  togglePYTHONSTARTUP() {
    [
      process.env._LAST_PYTHONSTARTUP,
      process.env.PYTHONSTARTUP,
    ] = [
      process.env.PYTHONSTARTUP || '',
      process.env._LAST_PYTHONSTARTUP || '',
    ];
    atom.notifications.addInfo(`PYTHONSTARTUP=${JSON.stringify(process.env.PYTHONSTARTUP)}`);
  }
  ...
  setLastOutputScreenPosition(position) {
    ...
    if (this.vmp) {
      this.vmp.getEditorState(this.editor).mark.set('h', this.lastOutputScreenPosition);
    }
  }

  scrollToLastOutput() {
    this.editor.scrollToScreenPosition(this.lastOutputScreenPosition);
  }
  ...
  async runAllCells(options = {}) {
    this.runCodeRanges(getCellRanges(this.editor, {all: true}));
  }
  async runAllCellsAbove(options = {}) {
    this.runCodeRanges(getCellRanges(this.editor, {above: true}));
  }
  async runAllCellsBelow(options = {}) {
    this.runCodeRanges(getCellRanges(this.editor, {below: true}));
  }
  async runAllCellsAboveAndCurrent(options = {}) {
    this.runCodeRanges(getCellRanges(this.editor, {above: true, current: true}));
  }
  async runAllCellsBelowAndCurrent(options = {}) {
    this.runCodeRanges(getCellRanges(this.editor, {below: true, current: true}));
  }

  async runCodeRanges(ranges, options = {}) {
      ...
      if (!kernelSpec) {
        ...
        return await new Promise((resolve, reject) => {
          ...
          atom.notifications.addInfo([
            `Starting kernel: ${kernelSpec.display_name}`,
            `- PYTHONSTARTUP=${JSON.stringify(process.env.PYTHONSTARTUP)}`,
          ].join('\n'));
...
class NotebookHydrogenResultReceiver {
  ...
  updateOutputPosition() {
    this.notebook.setLastOutputScreenPosition({
      ...
      //row: this.range.end.row + 1, // Buffer position

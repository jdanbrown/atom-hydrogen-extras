...
function rangeTrim(buffer, range) {
  ...
  addCommands() {
    this.disposables.add(
      atom.commands.add('atom-workspace', {
        ...
        'dan-notebook-v0:run-code-selection-or-line': ev => this.getNotebookForActiveTextEditor().runCodeSelectionOrLine(),
        ...
        'dan-notebook-v0:new-term-with-jupyter-for-current-kernel': ev => this.getNotebookForActiveTextEditor().newTermWithJupyterForCurrentKernel(),
        // TODO Move cmd + newTermWithCommand out of this package [and into what package?]
        'dan-notebook-v0:new-term-with-htop': ev => newTermWithCommand('sudo htop -d10'),
      })
    );
  },
...
class NotebookHydrogen {
  ...
  async runCode(options, range) {
    warnIfTextEditorIdsAreNotUnique();
    range = rangeTrim(this.editor.buffer, range);
  ...
  // TODO [Sketch] Auto-run code when its contents change
  ...
  async watchCode(options, range, markerLayer) {
    // TODO Where does markerLayer come from?
    const marker = markerLayer.markBufferRange(range, {
      class: 'watchcode-marker',
      invalidate: 'never', // TODO Should this really be 'never'?
    });

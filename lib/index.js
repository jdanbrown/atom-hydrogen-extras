...
const {CompositeDisposable} = require(`${atom.packages.resolvePackagePath('xterm')}/node_modules/event-kit`)
const hydrogen = {
...
class NotebookHydrogen {
  ...
  destroy() {
    ...
    // TODO Shutdown kernel (if exists)
  }
  ...
  async openJupyterTermForCurrentKernel() {
    ...
    if (kernel) {
      const connectionFile = await this.runCodeReturnOutput(
        // TODO dedent for multiline quotes `...`: https://github.com/dmnd/dedent (requires node_modules)
  ...
  ensureKernel() {
    if (!this._ensureKernelRunning) {
      this._ensureKernelRunning = this._ensureKernel().then(kernel => {
        this._ensureKernelRunning = undefined;
        return kernel;
      });
    }
    return this._ensureKernelRunning;
  }
...
class NotebookHydrogenResultReceiver {
  ...
  constructor(options, notebook, editor, range) {
    ...
    // // TODO TODO Figure out ipywidgets (?)
    // this.webview = document.createElement('webview');
    // this.webview.src = 'https://google.com';
    // [
    //   // Events: https://developer.mozilla.org/en-US/docs/Web/Events
    //   //  - TODO Allow some keys through, e.g. for switching panes? Not simple...
    //   //  - TODO Allow scroll events through? Prevent/accept inner scrollbar?
    //   // Clipboard
    //   'cut', 'copy', 'paste',
    //   // Keyboard
    //   'keydown', 'keypress', 'keyup',
    //   // Mouse
    //   'mouseenter', 'mouseover', 'mousemove', 'mousedown', 'mouseup', 'auxclick', 'click',
    //   'dblclick', 'contextmenu', 'wheel', 'mouseleave', 'mouseout', 'select',
    //   // Drag and drop
    //   'dragstart', 'drag', 'dragend', 'dragenter', 'dragover', 'dragleave', 'drop',
    // ].forEach(type => {
    //   this.webview.addEventListener(type, (ev) => {
    //     ev.stopPropagation();
    //   });
    // });
  ...
  onKernelResult(result) {
    ...
    if (result.stream === 'execution_count') {
      ...
      console.error('Unexpected kernel result:', result);
    }
  }
  ...
  resultStatus(status) {
    if (status === 'ok') {
      ...
      console.error(`Unexpected result.data[${status}] when result.stream[status]`);
    }
  }

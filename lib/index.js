    ...
    this.disposables.add(
      atom.workspace.observeTextEditors(editor => {
        if (!this.notebooks[editor.id]) {
          ...
          this.notebooks[editor.id] = new NotebookHydrogen(this, editor);
          this.disposables.add(
            editor.onDidDestroy(() => {
...
class NotebookHydrogen {
  ...
  setLastOutputScreenPosition(position) {
    ...
    if (this.trackOutput) {
      ...
      //        - How about just change to be start of next cell / para / non-blank line?
    ...
    if (this.hasVmp.vmp) {
      this.hasVmp.vmp.getEditorState(this.editor).mark.set('h', this.lastOutputScreenPosition);
    }
  }

...
export class KernelResultsReceiver {
  ...
  onKernelResultData(data) {
    ...
    if (result.data['text/html'] && !this.notebook.config.skipHtml) {
      ...
      // TODO Support more versions of vega/vegalite
      //  - This vegalite.v4 mimetype was added for altair 4.1.0
      //  - e.g. https://github.com/nteract/nteract/pull/4442/files
      this.resultAppendVegaliteV4FromJson(result.data['application/vnd.vegalite.v4+json']);
  ...
  async resultAppendVegaliteV4FromJson(vegaliteV4Json) {
    window.vegaliteV4Json = vegaliteV4Json; // XXX Debug

    const div = document.createElement('div');
    this.resultAppend(div);
    try {
      const vegaView = await vegaEmbed(div, vegaliteV4Json);
    ...
    // TODO Can we make vegalite openable?
    //  - Maybe one of these? https://vega.github.io/vega/docs/api/view/#image-export
    // this.initOpenable({
    //   elem: div,
    //   ...
    // });

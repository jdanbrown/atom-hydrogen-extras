  ...
  resultAppend(...elements) {
    ...
    Array.from(div.getElementsByTagName('script')).forEach(script => {
      if (script.type === 'module') {
        console.warn('<script type="module"> not yet supported, skipping', script);
        ...
        if (script.src) {
          ...
          console.warn('<script src=...> not yet supported, skipping', script);
        } else if (script.text) {
          if (script.async) console.warn('<script async> not yet supported, running immediately');
          if (script.defer) console.warn('<script defer> not yet supported, running immediately');
          // console.debug('<script> eval:', script.text.trim()); // XXX Debug
          setTimeout(
            // HACK This _might_ avoid some document.getElementById()->null race conditions on save/reload...
            () => {
              ...
              try {
                ...
                console.warn('<script> failed:', e.message, script);
                atom.notifications.addError(`&lt;script&gt; failed: ${e.message}`, {
                  detail: script.text,
                });
              }
            },
